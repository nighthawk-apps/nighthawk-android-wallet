    ...
    companion object {
        ...
        suspend fun create(
            ...
            return PersistableDarkfiWallet(
                ...
                network = network,
                endpoint = endpoint,
                // New wallets: 0 → UniFFI seeds scan cursor at LWD tip (no history).
                // Import via this helper still uses null → mapped to -1 (full scan);
                // restore UI path passes an explicit height or null instead.
                birthdayHeight =
                    when (mode) {
                        WalletInitMode.NewWallet -> 0L
                        is WalletInitMode.ImportWallet -> null
                    },
            )
        }
    }
