...
1. Install Android Studio and the Android SDK
    ...
    1. Configure `ANDROID_HOME` environment variable using the path noted above.  This will allow easily running Android development commands, like `adb logcat` or `adb install myapp.apk`
        1. macOS
            1. Default SDK location: `~/Library/Android/sdk`. Add to `~/.zprofile` **and** `~/.zshrc` (Cursor and some terminals only load `.zshrc`):
            ...
            1. Or copy the repo helper: `source scripts/android-sdk-env.sh` in your shell profile, or use the shared file `~/.android-sdk-env.sh` if you already created it during setup.
            1. Verify: open a **new** terminal tab and run `which adb` (should print `.../platform-tools/adb`).
