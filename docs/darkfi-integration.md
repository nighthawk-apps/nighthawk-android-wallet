...
`DarkfiEndpoint` mirrors **darkfid JSON-RPC** defaults from upstream [`bin/drk/drk_config.toml`](https://github.com/darkrenaissance/darkfi/blob/master/bin/drk/drk_config.toml):

| Network | darkfid TCP port (`endpoint =`) |
|---------|----------------------------------|
| mainnet | 8345 |
| testnet | 18345 |
| localnet (upstream only, not an app flavor yet) | 28345 |

Older APK builds used placeholder ports **26660 / 26670**; persisted JSON is auto-migrated on deserialization to match `drk_config.toml`.

Display strings use `tcp://host:port` (and `tcps://…` if TLS is ever enabled). The field is still named `tls` in JSON for backward compatibility.
...
| **`drk` → `darkfid` TCP ports** | **Aligned** | [`DarkfiEndpoint`](../darkfi-android-sdk/src/main/java/com/nighthawkapps/lib/android/sdk/wallet/DarkfiEndpoint.kt) defaults ↔ **`bin/drk/drk_config.toml`** `endpoint` (**8345 / 18345 / 28345**). Legacy **26660 / 26670** migrate away from persisted JSON (different from **`darkirc`** **`[rpc]`** defaults — documented in Known limitations). |
...
The in-app **Change server** flow edits presets (`DarkfiEndpointCatalog`) plus optional custom host/port and persists them independently from wallet seed JSON where configured.
