...
sealed class BackupStage(
    ...
    object Test : BackupStage(TEST_ORDER) {
        ...
        override fun getNext(): BackupStage = values[COMPLETE_ORDER]
    }

    object Failure : BackupStage(FAILURE_ORDER) {
        // To let user preview the seed again after test failure
        override fun getPrevious(): BackupStage = values[SEED_ORDER]
    }
...
private val sealedClassValues: List<BackupStage> =
    buildList<BackupStage> {
