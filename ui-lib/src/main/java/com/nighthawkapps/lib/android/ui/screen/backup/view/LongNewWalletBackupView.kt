...
import com.nighthawkapps.lib.android.ui.screen.backup.BackupTag
        ...
        when (backupState.current.collectAsStateWithLifecycle().value) {
            ...
            is BackupStage.Test -> {
                TestInProgress(
                    selectedTestChoices = choices,
                    onChoicesChanged = onChoicesChanged,
                    splitSeedPhrase = wallet.seedPhrase.toPersistentList(),
                    backupState = backupState
                )
            }
...
private fun SeedPhrase(persistableWallet: PersistableDarkfiWallet) {
    ...
    Column(
        Modifier
            .verticalScroll(rememberScrollState())
            .padding(vertical = WalletTheme.dimens.spacingDefault)
    ) {
                ...
                chunk.forEachIndexed { subIndex, word ->
                    ...
                    if (testIndices.contains(currentIndex)) {
                        ChipDropDown(
                            currentIndex,
                            dropdownText =
                                currentSelectedTestChoice[currentIndex]
                                    ?: "",
