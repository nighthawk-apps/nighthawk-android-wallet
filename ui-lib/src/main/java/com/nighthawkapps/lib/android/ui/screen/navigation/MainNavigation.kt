...
import com.nighthawkapps.lib.android.ui.screen.navigation.ArgumentKeys.DAO_NAME
import com.nighthawkapps.lib.android.ui.screen.navigation.ArgumentKeys.IS_PIN_SETUP
import com.nighthawkapps.lib.android.ui.screen.navigation.ArgumentKeys.PROPOSAL_BULLA
import com.nighthawkapps.lib.android.ui.screen.navigation.ArgumentKeys.TRANSACTION_DETAILS_ID
import com.nighthawkapps.lib.android.ui.screen.navigation.ArgumentKeys.VERIFY_PIN_TO_DISABLE
...
internal fun MainActivity.MainNavigation(
    ...
    NavHost(navController = navHostController, startDestination = BottomNavItem.Chat.route, modifier = Modifier.padding(paddingValues)) {
        ...
        composable(DAO_HUB) {
            AndroidDaoHub(
                onBack = { navHostController.popBackStackJustOnce(DAO_HUB) },
                onOpenDao = { name ->
                    navHostController.navigateJustOnce(NavigationTargets.navigationRouteDaoDetail(name))
                },
            )
        }
        composable(
            ...
            arguments =
                listOf(
                    navArgument(DAO_NAME) {
                        type = NavType.StringType
                        nullable = false
                    },
                ),
        ) {
            val daoName = Uri.decode(it.arguments?.getString(DAO_NAME).orEmpty())
            AndroidDaoDetail(
                daoName = daoName,
                onBack = { navHostController.popBackStack() },
                onOpenProposal = { bulla ->
                    navHostController.navigateJustOnce(NavigationTargets.navigationRouteDaoProposal(bulla))
                },
            )
        }
        composable(
            ...
            arguments =
                listOf(
                    navArgument(PROPOSAL_BULLA) {
                        type = NavType.StringType
                        nullable = false
                    },
                ),
        ) {
            val proposalBulla = Uri.decode(it.arguments?.getString(PROPOSAL_BULLA).orEmpty())
            AndroidDaoProposalDetail(
                proposalBullaB58 = proposalBulla,
                onBack = { navHostController.popBackStack() },
            )
        }
        composable(
        ...
        composable(
            ...
            arguments =
                listOf(
                    ...
                    navArgument(VERIFY_PIN_TO_DISABLE) {
                        type = NavType.BoolType
                        defaultValue = false
                    },
                )
            ...
            AndroidPin(
                ...
                verifyPinToDisable = it.arguments?.getBoolean(VERIFY_PIN_TO_DISABLE, false) ?: false,
                onBack = { navHostController.popBackStack() }
            )
        }
        ...
        composable(SECURITY) {
            AndroidSecurity(
                ...
                onDisablePin = {
                    navHostController.navigateJustOnce(
                        NavigationTargets.navigateToPinScreen(verifyToDisable = true),
                    )
                },
            )
        }
...
object NavigationTargets {
    ...
    const val PIN = "pin?$IS_PIN_SETUP={$IS_PIN_SETUP}&$VERIFY_PIN_TO_DISABLE={$VERIFY_PIN_TO_DISABLE}"
    const val DAO_HUB = "dao_hub"
    const val DAO_DETAIL = "dao_detail/{$DAO_NAME}"
    const val DAO_PROPOSAL = "dao_proposal/{$PROPOSAL_BULLA}"
    ...
    fun navigationRouteDaoDetail(daoName: String): String = DAO_DETAIL.replace("{$DAO_NAME}", Uri.encode(daoName))

    fun navigationRouteDaoProposal(proposalBullaB58: String): String =
        DAO_PROPOSAL.replace("{$PROPOSAL_BULLA}", Uri.encode(proposalBullaB58))

    fun navigateToPinScreen(
        isPinSetUp: Boolean = false,
        verifyToDisable: Boolean = false,
    ): String =
        PIN
            .replace("{$IS_PIN_SETUP}", "$isPinSetUp")
            .replace("{$VERIFY_PIN_TO_DISABLE}", "$verifyToDisable")
}

object ArgumentKeys {
    const val TRANSACTION_DETAILS_ID = "transactionId"
    const val IS_PIN_SETUP = "is_pin_setup"
    const val VERIFY_PIN_TO_DISABLE = "verify_pin_to_disable"
    const val DAO_NAME = "daoName"
    const val PROPOSAL_BULLA = "proposalBulla"
}
