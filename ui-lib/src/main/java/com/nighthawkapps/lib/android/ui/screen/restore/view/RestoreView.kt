...
private fun RestoreSeedMainContent(
    ...
    Column(
        ...
        if (!isSeedValid) {
            ...
            // Paste the whole phrase from the clipboard. Feeding the pasted text
            // through `setTextState` reuses `ParseResult`'s multi-word handling,
            // which validates each token against the BIP39 word list and appends
            // the matching words to `userWordList`.
            TertiaryButton(
                onClick = {
                    scope.launch {
                        clipboard.pastePlainText()?.let { pasted ->
                            val cleaned = pasted.trim()
                            if (cleaned.isNotEmpty()) {
                                setTextState(cleaned)
                            }
                        }
                    }
                },
