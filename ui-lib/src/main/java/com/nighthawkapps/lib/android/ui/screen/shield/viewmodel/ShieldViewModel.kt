package com.nighthawkapps.lib.android.ui.screen.shield.viewmodel
...
import com.nighthawkapps.lib.android.ui.screen.shield.model.ShieldUIState
import com.nighthawkapps.lib.android.ui.screen.shield.model.ShieldUiDestination
import com.nighthawkapps.lib.android.ui.screen.shield.model.ShieldingProcessState
...
class ShieldViewModel(
    ...
    fun checkAutoShieldUiState() {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            if (canAutoShield) {
                ...
                _shieldUiState.update {
                    ShieldUIState.OnResult(
                        ShieldUiDestination.AutoShieldError(
                            "Try again after ${maxAutoShieldFrequency.milliseconds.inWholeMinutes} minutes of your last auto shielding"
                        )
                    )
                }
            }
    ...
    fun getCurrentDestination(): ShieldUiDestination? =
        when (val uiState = _shieldUiState.value) {
    ...
    fun updateLastAutoShieldTime() {
        viewModelScope.launch(Dispatchers.IO) {
            val encryptionProvider = StandardPreferenceSingleton.getInstance(context)
            StandardPreferenceKeys.LAST_AUTOSHIELDING_PROMPT_EPOCH_MILLIS_STRING.putValue(
                encryptionProvider,
    ...
    private suspend fun canAutoShield(): Boolean {
        ...
        val encryptionProvider = StandardPreferenceSingleton.getInstance(context)
        val lastAutoShieldEpochMillis =
            StandardPreferenceKeys.LAST_AUTOSHIELDING_PROMPT_EPOCH_MILLIS_STRING
                .getValue(
                    encryptionProvider
                ).toLongOrNull()
